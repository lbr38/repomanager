...
You should backup this volume to ensure you can restore your data in case of a failure. **This can be basically done by copying the content of the volume to another location with rsync or cp**.

As the database is SQLite, no dump is needed.
...
This volumes contains the packages and metadata of the repositories.

You should backup this volume to ensure you can restore your data in case of a failure. **This can be basically done by copying the content of the volume to another location with rsync or cp**.

### In resume

In resume, you should backup both `repomanager-data` and `repomanager-repo` volume directories which are located:

- `/var/lib/docker/volumes/repomanager-data`
- `/var/lib/docker/volumes/repomanager-repo`
