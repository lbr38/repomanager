...
if ($opAction == "duplicate") {
    $options = getopt(null, ["id:", "newname:", "group:", "description:"]);
     ...
     */
    $op->repo = new Repo();
    $op->repo->setid($options['id']);
    $op->repo->setTargetName($options['newname']);
    $op->repo->setTargetGroup($options['group']);
    $op->repo->setTargetDescription($options['description']);
