...
class Env extends \Controllers\Task\Execution
{
    ...
    public function execute()
    {
        foreach ($this->repoController->getEnv() as $env) {
            ...
            // Check if an environment of the same name already points to the target snapshot
            if ($this->repoController->existsSnapIdEnv($this->repoController->getSnapId(), $env) === true) {
                if ($this->repoController->getPackageType() == 'rpm') {
                    throw new Exception(Label::envtag($env) . ' environment already points to <span class="label-white">' . $this->repoController->getName() . '</span>⸺<span class="label-black">' . $this->repoController->getDateFormatted() . '</span>');
                }

                if ($this->repoController->getPackageType() == 'deb') {
                    throw new Exception(Label::envtag($env) . ' environment already points to <span class="label-white">' . $this->repoController->getName() . ' ❯ ' . $this->repoController->getDist() . ' ❯ ' . $this->repoController->getSection() . '</span>⸺<span class="label-black">' . $this->repoController->getDateFormatted() . '</span>');
                }
            }
