    ...
    public function validate(array $formParams)
    {
        ...
        if ($myrepo->getPackageType() == 'deb') {
            if ($debRepoController->isActive($formParams['name'], $myrepo->getDist(), $myrepo->getSection())) {
                throw new Exception('<span class="label-white">' . $formParams['name'] . ' ❯ ' . $myrepo->getDist() . ' ❯ ' . $myrepo->getSection() . '</span> repository already exists');
            }
        }
         ...
         */
        if ($myrepo->getPackageType() == 'rpm') {
            History::set('Running task: renaming repository <span class="label-white">' . $myrepo->getName() . '</span>⸺<span class="label-black">' . $myrepo->getDateFormatted() . '</span> ➡ <span class="label-white">' . $formParams['name'] . '</span>');
        }
        if ($myrepo->getPackageType() == 'deb') {
            History::set('Running task: renaming repository <span class="label-white">' . $myrepo->getName() . ' ❯ ' . $myrepo->getDist() . ' ❯ ' . $myrepo->getSection() . '</span>⸺<span class="label-black">' . $myrepo->getDateFormatted() . '</span> ➡ <span class="label-white">' . $formParams['name'] . ' ❯ ' . $myrepo->getDist() . ' ❯ ' . $myrepo->getSection() . '</span>');
        }
