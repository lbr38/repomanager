...
class Create
{
    public function validate(array $formParams)
    {
         ...
         */
        if ($formParams['package-type'] == 'rpm') {
            $myhistory->set($_SESSION['username'], 'Run operation: New repository <span class="label-white">' . $targetName . '</span> (' . $formParams['repo-type'] . ')', 'success');
        }

        if ($formParams['package-type'] == 'deb') {
            foreach ($formParams['dist'] as $distribution) {
                foreach ($formParams['section'] as $section) {
                    $myhistory->set($_SESSION['username'], 'Run operation: New repository <span class="label-white">' . $targetName . ' ❯ ' . $distribution . ' ❯ ' . $section . '</span> (' . $formParams['repo-type'] . ')', 'success');
                }
            }
        }
    }
