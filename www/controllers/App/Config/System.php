...
class System
{
    ...
    public static function get()
    {
        ...
        /**
         *  Clear cookies starting with 'tables/', 'chart/' or 'temp/' when the page has been reloaded by the user (not AJAX)
         */
        if (AJAX === false) {
            foreach ($_COOKIE as $key => $value) {
                if (strpos($key, 'tables/') === 0 or strpos($key, 'chart') === 0 or strpos($key, 'temp/') === 0) {
