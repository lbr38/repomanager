...
class Settings
{
    ...
    public static function get()
    {
        ...
        //         $__LOAD_SETTINGS_MESSAGES[] = APP_YAML . ' file is not readable';
        //     } else {
        //         $app_yaml = yaml_parse_file(APP_YAML);

        //         // If there is an error while parsing the YAML file
        //         if ($app_yaml === false or empty($app_yaml)) {
        //             $__LOAD_SETTINGS_MESSAGES[] = 'Error while parsing ' . APP_YAML . ' file';
        //         }
        //     }
        // }
    ...
    public static function getYaml()
    {
         ...
         *  OIDC settings
         */
        if (!defined('OIDC_ENABLED') and isset($app_yaml['oidc']['enabled'])) {
            define('OIDC_ENABLED', $app_yaml['oidc']['enabled']);
        }

        if (!defined('SSO_OIDC_ONLY') and isset($app_yaml['oidc']['oidc_only'])) {
            define('SSO_OIDC_ONLY', $app_yaml['oidc']['oidc_only']);
        }

        if (!defined('OIDC_PROVIDER_URL') and isset($app_yaml['oidc']['provider_url'])) {
            define('OIDC_PROVIDER_URL', Common::validateData($app_yaml['oidc']['provider_url']));
        }

        if (!defined('OIDC_AUTHORIZATION_ENDPOINT') and isset($app_yaml['oidc']['authorization_endpoint'])) {
            define('OIDC_AUTHORIZATION_ENDPOINT', Common::validateData($app_yaml['oidc']['authorization_endpoint']));
        }

        if (!defined('OIDC_TOKEN_ENDPOINT') and isset($app_yaml['oidc']['token_endpoint'])) {
            define('OIDC_TOKEN_ENDPOINT', Common::validateData($app_yaml['oidc']['token_endpoint']));
        }

        if (!defined('OIDC_USERINFO_ENDPOINT') and isset($app_yaml['oidc']['userinfo_endpoint'])) {
            define('OIDC_USERINFO_ENDPOINT', Common::validateData($app_yaml['oidc']['userinfo_endpoint']));
        }

        if (!defined('OIDC_SCOPES') and isset($app_yaml['oidc']['scopes'])) {
            define('OIDC_SCOPES', Common::validateData($app_yaml['oidc']['scopes']));
        }

        if (!defined('OIDC_CLIENT_ID') and isset($app_yaml['oidc']['client_id'])) {
            define('OIDC_CLIENT_ID', Common::validateData($app_yaml['oidc']['client_id']));
        }

        if (!defined('OIDC_CLIENT_SECRET') and isset($app_yaml['oidc']['client_secret'])) {
            define('OIDC_CLIENT_SECRET', Common::validateData($app_yaml['oidc']['client_secret']));
        }

        if (!defined('OIDC_USERNAME') and isset($app_yaml['oidc']['username'])) {
            define('OIDC_USERNAME', Common::validateData($app_yaml['oidc']['username']));
        }

        if (!defined('OIDC_FIRST_NAME') and isset($app_yaml['oidc']['first_name'])) {
            define('OIDC_FIRST_NAME', Common::validateData($app_yaml['oidc']['first_name']));
        }

        if (!defined('OIDC_LAST_NAME') and isset($app_yaml['oidc']['last_name'])) {
            define('OIDC_LAST_NAME', Common::validateData($app_yaml['oidc']['last_name']));
        }

        if (!defined('OIDC_EMAIL') and isset($app_yaml['oidc']['email'])) {
            define('OIDC_EMAIL', Common::validateData($app_yaml['oidc']['email']));
        }

        if (!defined('OIDC_GROUPS') and isset($app_yaml['oidc']['groups'])) {
            define('OIDC_GROUPS', Common::validateData($app_yaml['oidc']['groups']));
        }

        if (!defined('OIDC_GROUP_ADMINISTRATOR') and isset($app_yaml['oidc']['group_administrator'])) {
            define('OIDC_GROUP_ADMINISTRATOR', Common::validateData($app_yaml['oidc']['group_administrator']));
        }

        if (!defined('OIDC_GROUP_SUPER_ADMINISTRATOR') and isset($app_yaml['oidc']['group_super_administrator'])) {
            define('OIDC_GROUP_SUPER_ADMINISTRATOR', Common::validateData($app_yaml['oidc']['group_super_administrator']));
        }
