...
class Permission extends \Controllers\User\User
{
    ...
    public function set(int $id, array $reposView, array $reposActions, array $tasksActions, array $hostsActions) : void
    {
        ...
        foreach (array_filter($reposView) as $repoOrGroup) {
            ...
            /**
             *  If entry starts with 'repo-', then it's an individual repository
             */
            if (strpos($repoOrGroup, 'repo-') === 0) {
                // Get repository Id
                $repoId = str_replace('repo-', '', $repoOrGroup);

                // Check that repository Id is valid
                if (!is_numeric($repoId)) {
                    throw new Exception('Invalid repository Id: ' . $repoId);
                }

                // Add repository Id key to permissions
                $permissions['repositories']['view']['repos'][] = $repoId;
            }
        }
