...
class Deb extends \Models\Model
{
    ...
    public function getAccess(string $name, string $dist, string $component, array $envs, int $timeStart, int $timeEnd, bool $count = false, bool $withOffset = false, int $offset = 0): array|int
    {
        ...
        try {
            ...
            // Build query
            $query .= " FROM access_deb WHERE Name = :name AND Dist = :dist AND Section = :component";
    ...
    public function getAccessByPeriod(string $name, string $dist, string $component, array $envs, int $timeStart, int $timeEnd) : array|int
    {
        ...
        try {
            $query = "SELECT Env, Timestamp, COUNT(*) as Count FROM access_deb WHERE Name = :name AND Dist = :dist AND Section = :component";
    ...
    public function getDailyAccessCount(string $name, string $dist, string $component, array $envs, int $timeStart, int $timeEnd): int
    {
        ...
        try {
            $query = "SELECT COUNT(*) as Count FROM access_deb WHERE Name = :name AND Dist = :dist AND Section = :component";
     ...
     *  It is possible to add an offset to the request
     */
    public function getAccessByIpCount(string $name, string $dist, string $component, array $envs, int $timeStart, int $timeEnd, bool $count, bool $withOffset, int $offset): array|int
    {
        $data = [];
    ...
    {
        try {
            $stmt = $this->db->prepare("INSERT INTO access_deb (Timestamp, Name, Dist, Section, Env, Source, IP, Request, Request_result) VALUES (:timestamp, :name, :dist, :component, :env, :sourceHost, :sourceIp, :request, :result)");
