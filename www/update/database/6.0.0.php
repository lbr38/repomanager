...
/**
 *  The processes of the tasks that were running or waiting in the queue did not survive the update,
 *  so those tasks would never end nor start again, and would hold a place in the queue forever.
 */
try {
    $this->db->exec("UPDATE tasks SET Status = 'stopped' WHERE Status = 'running' OR Status = 'queued'");
} catch (Exception $e) {
    throw new Exception('could not close the tasks interrupted by the update: ' . $e->getMessage());
}
