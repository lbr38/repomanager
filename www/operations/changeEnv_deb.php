...
function changeEnv_deb($repoName, $repoDist, $repoSection, $repoEnv, $repoNewEnv, $repoDescription) {
  global $REPOS_LIST;
  ...
  writeLog("<h5>NOUVEL ENVIRONNEMENT DE SECTION</h5>
  <table>
  <tr>
  <td>Section :</td>
  ...
  if (empty($repoDescription)) {
    $repoDescription = exec("egrep '^Name=\"${repoName}\",Host=\".*\",Dist=\"${repoDist}\",Section=\"${repoSection}\",Env=\"${repoNewEnv}\"' $REPOS_LIST | awk -F ',' '{print $7}' | cut -d'=' -f2 | sed 's/\"//g'");
  }
  ...
  $checkIfRepoExists = exec("egrep '^Name=\"${repoName}\",Host=\".*\",Dist=\"${repoDist}\",Section=\"${repoSection}\",Env=\"${repoNewEnv}\",Date=\"${repoDate}\"' $REPOS_LIST");
  if (!empty($checkIfRepoExists)) {
    $msg = "<tr><td colspan=\"100%\"><br><span class=\"redtext\">Erreur :</span> cette section est déjà en ${repoNewEnv} au ${repoDate}</td></tr>";
    writeLog("$msg"); // Enregistre l'erreur dans le log
    echo "$msg";      // Affiche l'erreur à l'utilisateur
    return false;
  }
  ...
  // TRAITEMENT 
	...
	// Deux cas possibles : 
	// - cette section n'avait pas de version dans l'environnement cible, on crée simplement un lien symbo
	// - cette section avait déjà une version dans l'environnement cible, on modifie le lien symbo et on passe la version précédente en archive
  $checkIfRepoExists = exec("egrep '^Name=\"${repoName}\",Host=\".*\",Dist=\"${repoDist}\",Section=\"${repoSection}\",Env=\"${repoNewEnv}\"' $REPOS_LIST");
  
  // Cas 1 : pas de version déjà en $repoNewEnv
  if (empty($checkIfRepoExists)) {
    // Suppression du lien symbolique (on sait jamais si il existe)
    if (file_exists("${REPOS_DIR}/${repoName}/${repoDist}/${repoSection}_${repoNewEnv}")) {
      unlink("${REPOS_DIR}/${repoName}/${repoDist}/${repoSection}_${repoNewEnv}");
    }

    // Création du lien symbolique
    exec("cd ${REPOS_DIR}/${repoName}/${repoDist}/ && ln -s ${repoDate}_${repoSection}/ ${repoSection}_${repoNewEnv}");

		// Mise à jour des informations dans repos.list :"
		file_put_contents("$REPOS_LIST", "Name=\"${repoName}\",Host=\"${repoHost}\",Dist=\"${repoDist}\",Section=\"${repoSection}\",Env=\"${repoNewEnv}\",Date=\"${repoDate}\",Description=\"${repoDescription}\"" . PHP_EOL, FILE_APPEND);
  }
  ...
  if (!empty($checkIfRepoExists)) {
    ...
    $old_repoDate = exec("egrep '^Name=\"${repoName}\",Host=\".*\",Dist=\"${repoDist}\",Section=\"${repoSection}\",Env=\"${repoNewEnv}\"' $REPOS_LIST | awk -F ',' '{print $6}' | cut -d'=' -f2 | sed 's/\"//g'");
    $old_repoDescription = exec("egrep '^Name=\"${repoName}\",Host=\".*\",Dist=\"${repoDist}\",Section=\"${repoSection}\",Env=\"${repoNewEnv}\"' $REPOS_LIST | awk -F ',' '{print $7}' | cut -d'=' -f2 | sed 's/\"//g'");
    if (!rename("${REPOS_DIR}/${repoName}/${repoDist}/${old_repoDate}_${repoSection}", "${REPOS_DIR}/${repoName}/${repoDist}/archived_${old_repoDate}_${repoSection}")) {
    ...
    $repos_list_content = preg_replace("/Name=\"${repoName}\",Host=\"${repoHost}\",Dist=\"${repoDist}\",Section=\"${repoSection}\",Env=\"${repoNewEnv}\",Date=\"${old_repoDate}\",Description=\".*\"/", "", $repos_list_content);
    file_put_contents("$REPOS_LIST", $repos_list_content);
		file_put_contents("$REPOS_LIST", "Name=\"${repoName}\",Host=\"${repoHost}\",Dist=\"${repoDist}\",Section=\"${repoSection}\",Env=\"${repoNewEnv}\",Date=\"${repoDate}\",Description=\"${repoDescription}\"" . PHP_EOL, FILE_APPEND);

		// Mise à jour des informations dans repos-archive.list :
    file_put_contents("$REPOS_ARCHIVE_LIST", "Name=\"${repoName}\",Host=\"${repoHost}\",Dist=\"${repoDist}\",Section=\"${repoSection}\",Date=\"${old_repoDate}\",Description=\"${old_repoDescription}\"" . PHP_EOL, FILE_APPEND);

    // Application des droits sur la section archivée
    exec("find ${REPOS_DIR}/${repoName}/${repoDist}/archived_${old_repoDate}_${repoSection}/ -type f -exec chmod 0660 {} \;");
    exec("find ${REPOS_DIR}/${repoName}/${repoDist}/archived_${old_repoDate}_${repoSection}/ -type d -exec chmod 0770 {} \;");
  }

	// Application des droits sur la section modifiée
	exec("find ${REPOS_DIR}/${repoName}/${repoDist}/${repoDate}_${repoSection}/ -type f -exec chmod 0660 {} \;");
  exec("find ${REPOS_DIR}/${repoName}/${repoDist}/${repoDate}_${repoSection}/ -type d -exec chmod 0770 {} \;");
