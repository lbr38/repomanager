...
function changeEnv_rpm($repoName, $repoEnv, $repoNewEnv, $repoDescription) {
  ...
  if (empty($checkIfRepoExists)) {
    ...
    exec("cd ${REPOS_DIR}/ && ln -s ${repoDate}_${repoName}/ ${repoName}_${repoNewEnv}");
		...
		file_put_contents("$REPOS_LIST", "Name=\"${repoName}\",Realname=\"${repoRealname}\",Env=\"${repoNewEnv}\",Date=\"${repoDate}\",Description=\"${repoDescription}\"" . PHP_EOL, FILE_APPEND);
  }
  ...
  if (!empty($checkIfRepoExists)) {
    ...
    $repos_list_content = preg_replace("/Name=\"${repoName}\",Realname=\"${repoRealname}\",Env=\"${repoNewEnv}\",Date=\"${old_repoDate}\",Description=\".*\"/", "", $repos_list_content);
    file_put_contents("$REPOS_LIST", $repos_list_content);
		file_put_contents("$REPOS_LIST", "Name=\"${repoName}\",Realname=\"${repoRealname}\",Env=\"${repoNewEnv}\",Date=\"${repoDate}\",Description=\"${repoDescription}\"" . PHP_EOL, FILE_APPEND);

		// Mise à jour des informations dans repos-archive.list :
    file_put_contents("$REPOS_ARCHIVE_LIST", "Name=\"${repoName}\",Realname=\"${repoRealname}\",Date=\"${old_repoDate}\",Description=\"${old_repoDescription}\"" . PHP_EOL, FILE_APPEND);
