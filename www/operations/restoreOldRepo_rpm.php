...
function restoreOldRepo_rpm($repoName, $repoDate, $repoEnv, $repoDescription) {
    ...
    $repoRealname = exec("egrep '^Name=\"${repoName}\",Realname=\".*\",Date=\"${repoDate}\"' $REPOS_ARCHIVE_LIST | awk -F ',' '{print $2}' | cut -d'=' -f2 | sed 's/\"//g'");
    ...
    if (!empty($checkIfStillUsed)) {
        ...
        file_put_contents("$REPOS_LIST", "Name=\"${repoName}\",Realname=\"${repoRealname}\",Env=\"${repoEnv}\",Date=\"${repoDate}\",Description=\"${repoDescription}\"".PHP_EOL , FILE_APPEND | LOCK_EX);
        ...
        $content = preg_replace("/Name=\"${repoName}\",Realname=\"${repoRealname}\",Date=\"${repoDate}\",Description=\".*\"/", "", $content);
