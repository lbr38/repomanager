        ...
        if ($this->signed == "yes" OR $this->gpgResign == "yes") {
            ...
            if ($OS_FAMILY == "Redhat") {
                ...
                if (file_exists("/usr/bin/rpmresign")) {
                    ...
                    $process = proc_open("shopt -s globstar && cd '${REPOS_DIR}/${DATE_JMA}_{$this->name}' && /usr/bin/rpmresign --path '${GPGHOME}' --name '${GPG_KEYID}' --passwordfile '${PASSPHRASE_FILE}' **/*.rpm 1>&2", $descriptors, $pipes);
            ...
            if ($OS_FAMILY == "Debian") {
                ...
                if (!mkdir("${REPOS_DIR}/{$this->name}/{$this->dist}/${DATE_JMA}_{$this->section}/conf", 0770, true)) {
                ...
                if (!file_put_contents("${REPOS_DIR}/{$this->name}/{$this->dist}/${DATE_JMA}_{$this->section}/conf/options", "basedir ${REPOS_DIR}/{$this->name}/{$this->dist}/${DATE_JMA}_{$this->section}\nask-passphrase".PHP_EOL)) {
                ...
                $process = proc_open("exec /usr/bin/reprepro --basedir ${REPOS_DIR}/{$this->name}/{$this->dist}/${DATE_JMA}_{$this->section}/ --gnupghome ${GPGHOME} includedeb {$this->dist} ${TMP_DIR}/*.deb 1>&2", $descriptors, $pipes);
            }
            ...
            if ($return == 0) {
                ...
                if ($OS_FAMILY == "Debian") { echo "<br><span class=\"redtext\">Erreur : </span>la signature de la section <b>$this->section</b> du repo <b>$this->name</b> a échouée"; }
