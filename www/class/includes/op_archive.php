...
trait op_archive {
    ...
    public function op_archive() {
        ...
        if ($OS_FAMILY == "Debian") $stmt = $this->db->prepare("SELECT * FROM repos WHERE Name=:name AND Dist=:dist AND Section=:section AND Date=:date AND Env !=:env AND Status = 'active'");
