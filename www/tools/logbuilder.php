...
if (!empty($argv[1])) {
    $PIDFILE = $argv[1];              // Emplacement du fichier PID principal
}
if (!empty($argv[2])) {
    $LOGFILE = $argv[2];              // Chemin du fichier de log principal (logs/main/repomanager...)
}
if (!empty($argv[3])) {
    $OPERATION_TEMP_DIR = $argv[3];   // Chemin du répertoire temporaire de l'opération en cours (.temp/PID/)
}
if (!empty($argv[4])) {
    $steps = $argv[4];                // Nombre d'étapes totales
}
