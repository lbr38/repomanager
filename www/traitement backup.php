...
// 2ème étape, si on a toutes les variables, on demande une confirmation puis si on a la confirmation alors on lance l'exécution
    ...
    // Cas Debian :
    if ($OS_FAMILY == "Debian") {
        if (!empty($repoHostName) AND !empty($repoAlias) AND !empty($repoDist) AND !empty($repoSection) AND !empty($repoDescription) AND !empty($newRepoGpgCheck) AND !empty($repoGpgResign)) {
...
// 1ère étape : on vérifie qu'on a bien reçu toutes les variables nécéssaires en GET :
    ...
    // Si $repoGpgResign est vide, on le demande
...
// 2ème étape, si on a toutes les variables, on demande une confirmation puis si on a la confirmation alors on lance l'exécution
    ...
    // Cas Debian :
    if ($OS_FAMILY == "Debian") {
        if (!empty($repoName) AND !empty($repoDist) AND !empty($repoSection) AND !empty($updateRepoGpgCheck) AND !empty($repoGpgResign)) {
            ...
            if (!empty($_GET['confirm']) AND (validateData($_GET['confirm']) == "yes")) {
                echo '<tr><td>Chargement <img src="images/loading.gif" class="icon" /></td></tr>';
                exec("${REPOMANAGER} --web --updateRepo --gpg-check $updateRepoGpgCheck --gpg-resign $repoGpgResign --repo-name $repoName --repo-host-name $repoHostName --repo-dist $repoDist --repo-section $repoSection >/dev/null 2>/dev/null &");
                echo "<script>window.location.replace('/journal.php');</script>"; // Dans les deux cas on redirige vers la page de logs pour voir l'exécution
            }
