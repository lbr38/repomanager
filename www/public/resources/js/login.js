...
$(document).on('click','#user-generate-apikey-btn',function () {
    confirmBox(
        ...
        function () {
            ajaxRequest('login', 'generateApikey', {}, true, true, [], [ "$('.slide-panel-container[slide-panel=\"general/userspace\"]').find('#user-apikey').html(jsonValue.message.apikey);", "$('.slide-panel-container[slide-panel=\"general/userspace\"]').find('#user-apikey').addClass('copy');" ])
        },
        'Generate'
    );
