...
function printDetails(array $variables = []) {
    ...
    if ($OS_FAMILY == "Redhat") {
        ...
        $checkIfRepoExists = exec("egrep '^Name=\"${repoName}\",Realname=\"${repoSource}\",Env=\"${DEFAULT_ENV}\"' $REPOS_LIST");
        ...
        $checkIfRepoExists = exec("egrep '^Name=\"${repoName}\",Realname=\"${repoSource}\",Env=\"${DEFAULT_ENV}\",Date=\"${DATE_JMA}\"' $REPOS_LIST");
        if (!empty($checkIfRepoExists)) {
    ...
    if ($OS_FAMILY == "Debian") {
        ...
        $checkIfRepoExists = exec("egrep '^Name=\"${repoName}\",Host=\"${repoSource}\",Dist=\"${repoDist}\",Section=\"${repoSection}\",Env=\"${DEFAULT_ENV}\"' $REPOS_LIST");
        if (empty($checkIfRepoExists)) {
        ...
        $checkIfRepoExists = exec("egrep -q '^Name=\"${repoName}\",Host=\"${repoSource}\",Dist=\"${repoDist}\",Section=\"${repoSection}\",Env=\"${DEFAULT_ENV}\",Date=\"${DATE_JMA}\"' $REPOS_LIST");
        if (!empty($checkIfRepoExists)) {
	    ...
	    if ($OS_FAMILY == "Debian") {
            $hostFullUrl = exec("grep '^Name=\"${repoSource}\",Url=' $HOSTS_CONF | awk -F ',' '{print $2}' | cut -d'=' -f2 | sed 's/\"//g'"); // Récupère l'url complète
    ...
    if ($OS_FAMILY == "Redhat") { $oldRepoDate = exec("egrep '^Name=\"${repoName}\",Realname=\"${repoSource}\",Env=\"${DEFAULT_ENV}\"' $REPOS_LIST | awk -F ',' '{print $4}' | cut -d'=' -f2 | sed 's/\"//g'"); }
    if ($OS_FAMILY == "Debian") { $oldRepoDate = exec("egrep '^Name=\"${repoName}\",Host=\"${repoSource}\",Dist=\"${repoDist}\",Section=\"${repoSection}\",Env=\"${DEFAULT_ENV}\"' $REPOS_LIST | awk -F ',' '{print $6}' | cut -d'=' -f2 | sed 's/\"//g'"); }
...
function getPackages(array $variables = []) {
	...
	if ($OS_FAMILY == "Redhat") {
		if ($repoGpgCheck == "no") {
			if ($OS_VERSION == "7") {
				exec("cd '${REPOS_DIR}/${DATE_JMA}_${repoName}' && reposync --config=${REPOMANAGER_YUM_DIR}/repomanager.conf -l --repoid=${repoSource} --norepopath --download_path='${REPOS_DIR}/${DATE_JMA}_${repoName}/' >> $stepLog", $output, $result);
			}
			if ($OS_VERSION == "8") {
				exec("cd '${REPOS_DIR}/${DATE_JMA}_${repoName}' && reposync --config=${REPOMANAGER_YUM_DIR}/repomanager.conf --nogpgcheck --repoid=${repoSource} --download-path '${REPOS_DIR}/${DATE_JMA}_${repoName}/' >> $stepLog", $output, $result);
			}
		} else { // Dans tous les autres cas (même si rien n'a été précisé) on active gpgcheck
			if ($OS_VERSION == "7") {
				exec("cd '${REPOS_DIR}/${DATE_JMA}_${repoName}' && reposync --config=${REPOMANAGER_YUM_DIR}/repomanager.conf --gpgcheck -l --repoid=${repoSource} --norepopath --download_path='${REPOS_DIR}/${DATE_JMA}_${repoName}/' >> $stepLog", $output, $result);
			}
			if ($OS_VERSION == "8") {
				exec("cd '${REPOS_DIR}/${DATE_JMA}_${repoName}' && reposync --config=${REPOMANAGER_YUM_DIR}/repomanager.conf --repoid=${repoSource} --download-path '${REPOS_DIR}/${DATE_JMA}_${repoName}/' >> $stepLog", $output, $result);
			}
		}
		echo '</pre></div>';
...
function updateReposLists(array $variables = []) {
	...
	if ($OS_FAMILY == "Redhat") {
        ...
        if (!file_put_contents($REPOS_LIST, preg_replace("/Name=\"${repoName}\",Realname=\".*\",Env=\"${DEFAULT_ENV}\",Date=\"${oldRepoDate}\"/", "Name=\"${repoName}\",Realname=\"${repoSource}\",Env=\"${DEFAULT_ENV}\",Date=\"${DATE_JMA}\"", $repos_list_content))) {
	...
	if ($OS_FAMILY == "Debian") {
        $repos_list_content = file_get_contents($REPOS_LIST);
        if (!file_put_contents($REPOS_LIST, preg_replace("/Name=\"${repoName}\",Host=\".*\",Dist=\"${repoDist}\",Section=\"${repoSection}\",Env=\"${DEFAULT_ENV}\",Date=\"${oldRepoDate}\"/", "Name=\"${repoName}\",Host=\"${repoSource}\",Dist=\"${repoDist}\",Section=\"${repoSection}\",Env=\"${DEFAULT_ENV}\",Date=\"${DATE_JMA}\"", $repos_list_content))) {
...
if (!empty($argv)) {
  if ($OS_FAMILY == "Redhat") {
	...
	if (!empty($argv[4])) { $repoSource = $argv[4]; } else { throw new Exception("Erreur : vrai nom du repo non défini"); }
  ...
  if ($OS_FAMILY == "Debian") {
	...
	if (!empty($argv[6])) { $repoSource = $argv[6]; } else { throw new Exception("Erreur : hostname non défini"); }
 ...
 */
 ...
 try {
	...
	if ($OS_FAMILY == "Redhat") { $oldRepoDate = printDetails(compact('stepLog', 'repoName', 'repoSource', 'repoGpgCheck', 'repoGpgResign'));}
 	if ($OS_FAMILY == "Debian") { list($oldRepoDate, $repoHost, $repoRoot) = printDetails(compact('stepLog', 'repoName', 'repoDist', 'repoSection', 'repoSource', 'repoGpgCheck', 'repoGpgResign'));}
 ...
 */
 ...
 try {
	...
	if (!file_exists($stepLog)) { touch($stepLog); }
	if ($OS_FAMILY == "Redhat") { getPackages(compact('stepLog', 'repoName', 'repoSource', 'repoGpgCheck'));}
	if ($OS_FAMILY == "Debian") { getPackages(compact('stepLog', 'repoName', 'repoDist', 'repoSection', 'repoSource', 'repoGpgCheck', 'repoHost', 'repoRoot'));}
 ...
 */
 ...
 try {
	...
	if (!file_exists($stepLog)) { touch($stepLog); }
	if ($OS_FAMILY == "Redhat") { createRepo(compact('stepLog', 'repoName', 'repoSource'));}
	if ($OS_FAMILY == "Debian") { createRepo(compact('stepLog', 'repoName', 'repoDist', 'repoSection', 'repoSource'));}
...
try {
	...
	if (!file_exists($stepLog)) { touch($stepLog); }
	if ($OS_FAMILY == "Redhat") { archiveOldRepo(compact('stepLog', 'repoName', 'repoSource', 'oldRepoDate'));}
	if ($OS_FAMILY == "Debian") { archiveOldRepo(compact('stepLog', 'repoName', 'repoDist', 'repoSection', 'repoSource', 'oldRepoDate'));}
...
try {
	...
	if (!file_exists($stepLog)) { touch($stepLog); }
	if ($OS_FAMILY == "Redhat") { updateReposLists(compact('stepLog', 'repoName', 'repoSource', 'oldRepoDate'));}
	if ($OS_FAMILY == "Debian") { updateReposLists(compact('stepLog', 'repoName', 'repoDist', 'repoSection', 'repoSource', 'oldRepoDate'));}
